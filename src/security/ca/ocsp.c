...
int SSL_CheckCertStatus(char** _cSysCmd)
{
#ifdef CALLTRACE
    ...
    while (!badarg && *args && *args[0] == '-') {
	...
	else if (!strcmp(*args, "-certfile"))
        {
            if (args[1]) {
                ...
                X509_free(cert);
		printf("cert name %s\r\n", *args);
                cert = load_cert(bio_err, *args, FORMAT_ASN1,
                                 NULL, e, "certificate");
		if (!cert)
                    goto end;
                if (!cert_id_md)
...
#endif
//add by cfl 20160931
ret = print_ocsp_summary(out, bs, req, reqnames, ids, nsec, maxage);
 end:
    ERR_print_errors(bio_err);
    X509_free(signer);
...
/*
	#define V_OCSP_GERTSTATUS_GOOD 0
	#define V_OCSP_GERTSTATUS_REVOKED 1
	#define V_OCSP_GERTSTATUS_UNKNOWN 2
*/        
