...
int	compressfile(FILE* in , FILE* out , size_t* size_in , size_t* size_out);
...
int compressfile(FILE* in , FILE* out , size_t* size_in , size_t* size_out)
{
	...
	src = malloc(LZ4_BUF_SIZE);
	if (!src) {
		printf("Not enough memory");
		goto cleanup;
	}

	frame_size = LZ4F_compressBound(LZ4_BUF_SIZE, &lz4_preferences);
...
int Compress_File(const char* input , const char* output)
{
	...
	r = compressfile(in, out, &size_in, &size_out);
	if (r == 0)
